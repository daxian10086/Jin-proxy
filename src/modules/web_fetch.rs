// 缂傚倸鍟崹鍧楀Υ婢舵劕绠柟鎯у暱缁插潡鏌ㄥ☉娆戞憢RL 濠碘槅鍋€閸嬫挻绻涢弶鎴剭闁逞屽厸缁舵岸銆傞埡鍛煑闁哄诞鍕伅婵炲濯寸徊鍧楀箖婵犲嫮妫憸搴㈡叏閸曨垰鐭楅柡宥冨劗閸?

// 闁诲海鏁搁幊鎾惰姳?Python: jindx/web_fetch.py
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?SSRF 闂傚倸鍟鑸垫叏閵忋倖鏅慨姗嗗墮閺佸墎绱?IP 濠电偛鐗忛弫鑽ゅ垝閿曞倸瑙︾€广儱鎳庣粈?闂佸啿鍘滈崑鎾绘煃閸忓浜?
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?web_fetch 閻庤鎮堕崕閬嶅矗鐠恒劉鍋撶憴鍕叝缂?闂佸啿鍘滈崑鎾绘煃閸忓浜?
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?URL 闂佸湱绮崝鏇°亹?闂佸啿鍘滈崑鎾绘煃閸忓浜?
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?闂佸憡鑹鹃張顒勵敆?URL 闂佺鍩栭幐姝屻亹?闂佸啿鍘滈崑鎾绘煃閸忓浜?
    ...
    match client.get(url).send() {
        ...
        Ok(resp) => {
            ...
            let mut processed = if ct.contains("html") {
                ...
                // 缂備胶濮崑鎾绘煕濡や焦绀堟繛?HTML 闂佸搫绉村ú銊╊敆閻戣棄绀堥柕澹瞼鈧?

                let re_script = Regex::new(r"(?i)<script[^>]*>.*?</script>").unwrap();

                let re_style = Regex::new(r"(?i)<style[^>]*>.*?</style>").unwrap();
...
fn inject_fetched_context(messages: &mut Vec<Value>, fetched: &HashMap<String, String>) {
    ...
    let context = format!("\n\n[Pre-fetched web content 闂?use this directly, no need to call web_fetch]\n\n{}", context);
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?闂佺娴氶崜娆戞閹寸儑绱ｉ柛鏇ㄥ亜缁插潡鏌涢幋锝呅撻柡?闂佸啿鍘滈崑鎾绘煃閸忓浜?
...
// 闂佸啿鍘滈崑鎾绘煃閸忓浜?閻庢鍠栭崐褰掝敆?web_fetch 闂佸湱鐟抽崱鈺傛杸 闂佸啿鍘滈崑鎾绘煃閸忓浜?
