// HTTP閵嗕讣SE 閸?WebSocket API 鐠侯垳鏁遍妴?
...
async fn get_http_client() -> reqwest::Client {
...
async fn stream_chat(body: Value, auth_headers: &[(String, String)]) -> HttpResponse {
...

    {
        ...
        Err(e) => {
            ...
            return HttpResponse::Ok().content_type("text/event-stream").body(error_bytes);
...
}// 鏉╄棄濮為崚?routes.rs 閺堫偄鐔敍灞藉彆瀵偓 HTTP 鐎广垺鍩涚粩顖欎簰娓?main.rs 娑?Claude handler 娴ｈ法鏁?



lazy_static::lazy_static! {

    pub static ref HTTP_CLIENT_INTERNAL: tokio::sync::Mutex<Option<reqwest::Client>> = tokio::sync::Mutex::new(None);
